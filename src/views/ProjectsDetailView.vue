...
<template>
    <main class="flex flex-col gap-6">
        ...
        <div v-if="project.img && project.img.length > 0" class="flex flex-wrap gap-4">
            <img v-for="img in project.img" :src="`/img/projects/${route.params.slug}/${img}`" :alt="project.name" class="rounded shadow h-auto max-h-96 w-[24rem] max-w-full object-contain bg-black/20" />
        </div>
    </main>
</template>
